...
- **Assistant orb**: a persistent desktop badge bottom-left (above the
  volume badge) — blue when idle, amber while connecting, green and
  pulsing when the assistant is listening, red on error. **Click toggles
  the voice session — or just say "Hi Computer".** When hot it doubles in
  size with a bright ring on a dark contrast disc and never fades; when
  idle it shrinks and fades out after ~3 s with the other badges. It
  floats above every window, streaming sessions and VLC included.
  Sessions hang up automatically after 30 s without input (and whenever
  playback starts) — the orb can be woken again any time.
  Behind it is a **pure chatbot** (Gemini Live speech-to-speech, voice
  Leda): it chats, answers questions, tells stories and remembers the
  household across sessions — no kiosk control, by design. A rising chime
  marks activation, a falling one sleep; when the API quota runs out it
  says so. (A tools-capable cascade backend — media, UI, web, radio
  control — is parked in Settings → Assistant → Backend.) Behind it is a Gemini
...
- **Photos**: slideshow reads a local folder; import via **Settings →
  ...
  **multi-select batch delete** (Select → tap photos → Delete N, two-tap
  confirm), two-tap purge to a recoverable `.trash/`, in-place 90°
  rotate, and a blank-photo pre-filter (`bin/scan-blanks.php` scores
  luminance variance via ffmpeg; Find blanks → Purge N blank in the grid).
