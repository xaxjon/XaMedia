...
- **Realtime-paced delivery**: voice answers on gemini-3.1-flash-live-preview arrive ~1.0× realtime (just-in-time); text-turn answers burst at ~3.7×. Just-in-time delivery makes per-chunk scheduling stutter on any network jitter — assistant.js therefore runs a 350ms jitter buffer (prime → play → flush tail at generationComplete/turnComplete → re-prime per turn).
- **Dropped turnComplete (2026-09-21)**: under load the API streams the answer + `generationComplete` but never sends `turnComplete`; the session then ignores further user turns (looks dead but isn't closed). Handle: flush the jitter tail on generationComplete (or short answers sit unheard — the "1s drops then stall"), and rebuild the session if turnComplete doesn't follow within 8s. Also: `waitingForInput` keepalives refresh downstream traffic while the model is stuck — watchdogs must key off real output vs user speech, not lastRx.
- **Model availability on this key** (probed 2026-09-21): ONLY `gemini-3.1-flash-live-preview` works end-to-end (setup + tools + audio + turnComplete). `gemini-3.8-live` eats post-tool turns; 2.5-flash-live-preview, 2.5 native-audio variants, 3.7/3.8-flash-live all reject the setup (close 1000). The voice model is a **Settings option** (`assistant.live_model` in data/settings.json, Settings → Assistant → Voice model) — when a Google update might have fixed 3.8-live, run the wire test (toolCall → toolResponse → audio chunks?) and flip it from the couch if audio comes back.
