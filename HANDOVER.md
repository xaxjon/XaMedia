...
- **Room noise kills aged sessions** (the big one): constant TV "activity" fills the session context window until the model fades (answers the TV, incoherent) then goes mute with the socket open. Fresh sessions always work — that's the tell. Defenses deployed: client-side mic gate in assistant.js (attenuates below-RMS-threshold audio 40× — keep streaming, VAD needs trailing silence; thresholds GATE_RMS 0.030/0.020 may need per-room tuning), `contextWindowCompression` sliding window, and a deaf-upstream watchdog arm (gate sees speech but no transcription for 30s → rebuild).
